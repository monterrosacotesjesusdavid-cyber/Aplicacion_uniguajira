...
class CamaraScreen extends StatefulWidget {
  ...
  // Retorna String base64 o null si canceló
  static Future<String?> abrir(BuildContext context) async {
    ...
    return Navigator.push<String>(
      context,
      MaterialPageRoute(builder: (_) => CamaraScreen._(camera: front)),
    );
  }

  final CameraDescription? _camera;
  const CamaraScreen._({CameraDescription? camera}) : _camera = camera;
...
class _CamaraState extends State<CamaraScreen> {
  ...
  void initState() {
    super.initState();
    _ctrl = CameraController(widget._camera!, ResolutionPreset.medium, enableAudio: false);
